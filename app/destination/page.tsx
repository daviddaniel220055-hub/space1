...
function DestinationContent() {
  ...
  const selected =
    searchParams.get("planet")?.toUpperCase() ?? "MARS";

  const planet =
    planetData[selected as keyof typeof planetData] ??
    planetData.MARS;
    ...
    >
      <header className="flex items-center pt-4 sm:pt-8 relative z-20 px-4 sm:px-8 lg:px-12">
        ...
        <nav className="bg-white/5 backdrop-blur-md px-3 py-3 sm:px-6 sm:py-4 md:px-12 md:py-5 flex gap-3 sm:gap-6 md:gap-10 lg:gap-12 uppercase tracking-widest text-[10px] sm:text-xs md:text-sm overflow-x-auto">
          <Link
            href="/"
            className="text-white/70 flex items-center gap-1 sm:gap-2 hover:text-white hover:border-b-2 hover:border-white pb-1 -mb-1 transition-all whitespace-nowrap"
          >
            <span className="font-bold text-white/40 text-[8px] sm:text-xs">
              00
            </span>
            HOME
          </Link>
          ...
          <Link
            href="/destination?planet=mars"
            className="text-white flex items-center gap-1 sm:gap-2 border-b-2 border-white pb-1 -mb-1 transition-all whitespace-nowrap"
          >
            <span className="font-bold text-white/60 text-[8px] sm:text-xs">
              01
            </span>
            DESTINATION
          </Link>
